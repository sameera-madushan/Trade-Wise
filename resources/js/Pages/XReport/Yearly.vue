...
onMounted(async () => {
  ...
  options.ajax.url = `${$APP_URL}/user/${props.uuid}/report/get-yearly-report-data/${formattedYear}`;
...
watch(year, async (newYear) => {
  if (dataTable) {
    ...
    const newUrl = `${$APP_URL}/user/${props.uuid}/report/get-yearly-report-data/${formattedYear}`;
